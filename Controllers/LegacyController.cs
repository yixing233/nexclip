...
public class LegacyController(ClipboardService svc) : ControllerBase
{
    ...
    public async Task<IActionResult> Put([FromBody] LegacyUpload body)
    {
        ...
        var (_, _) = await svc.UploadTextAsync(body.Text, body.DeviceId ?? "legacy", body.DeviceName ?? "Legacy Client", null, null, HttpContext.Connection.RemoteIpAddress?.ToString());
